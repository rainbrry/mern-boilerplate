...
import { useGetReturnSalesReportsQuery } from "../../../services/api/reports";

export default function ReturnSalesReport() {
	const {
		data: returnSalesReports = [],
		isLoading,
		isError,
	} = useGetReturnSalesReportsQuery();

	const returnedTotal = returnSalesReports.reduce((acc, curr) => {
					...
					<div id="report" className="">
						<table className="table-fixed w-full border-r border-l border-t">
							...
							<tbody className="text-md divide-y text-left divide-gray-100 overflow-hidden">
								...
								) : isError || !returnSalesReports.length ? (
									<tr>
										<td className="p-4 whitespace-nowrap" colSpan="5">
											<div className="text-center text-lg font-semibold mt-56">
												No data
