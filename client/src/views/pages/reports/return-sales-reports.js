...
import { getReturnsReports } from "../../../redux/features/reportsSlice";

export default function ReturnSalesReports() {
	const dispatch = useDispatch();
	const { returnReports } = useSelector((state) => state.reports);

	const returnedTotal = returnReports.reduce((acc, curr) => {
		const total = curr.details.reduce((acc, curr) => {
			return acc + curr.price * curr.returnQty;
		}, 0);
		return acc + total;
	}, 0);

	useEffect(() => {
		dispatch(getReturnsReports());
	}, [dispatch]);

	return (
		<div className="px-8 py-4">
			<h3 className="text-2xl text-base-500">Laporan pembelian</h3>
			<div className="flex flex-row justify-end">
				<button
					onClick={() => generatePDF("Laporan-retur-penjualan.pdf")}
									...
									returnReports.map((report) =>
										report.details.map((detail, index) => (
											<tr key={index} className="hover:bg-gray-300">
												...
												<td className="px-4 py-2 whitespace-nowrap border-b text-center w-2/12">
													{rupiah(detail.price * detail.returnQty)}
												</td>
											</tr>
										))
									)
									...
									<tr className="bg-gray-50">
										...
										>
											<div className="font-semibold">
												Total refund : {rupiah(returnedTotal)}
											</div>
										</td>
									</tr>
								</tfoot>
