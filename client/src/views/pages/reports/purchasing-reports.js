...
import { getPurchasingReports } from "../../../redux/features/reportsSlice";
import { rupiah } from "../../../helpers/currency";
import { generatePDF } from "../../../helpers/generate-pdf";

export default function PurchasingReports() {
	const dispatch = useDispatch();
	const { purchasingReports } = useSelector((state) => state.reports);

	const totalPurchasing = purchasingReports.reduce(
		(acc, curr) => acc + curr.grandTotal,
		0
	);

	useEffect(() => {
		dispatch(getPurchasingReports());
	}, [dispatch]);
	...
	return (
		<div className="px-8 py-4">
			...
			<div className="flex flex-row justify-end">
				<button
					onClick={() => generatePDF("Laporan-pembelian.pdf")}
										...
										<tr className="bg-gray-50">
											...
											>
												<div className="font-semibold">
													Total pembelian : {rupiah(totalPurchasing)}
												</div>
											</td>
										</tr>
									</tfoot>
